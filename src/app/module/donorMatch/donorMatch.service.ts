...
const matchDonors = async (
  ...
  if (requester.role !== Role.REQUESTER) {
    throw new Error(
      "Only requesters can find donors for a blood request.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Donors cannot be matched for this request.",
    );
  }
  ...
  const compatibleGroups =
    compatibleDonorGroups[bloodRequest.bloodGroup];
  ...
  for (const donor of donors) {
    ...
    const existingMatch =
      await prisma.donorMatch.findUnique({
        where: {
          requestId_donorId: {
            requestId,
            donorId: donor.id,
...
const getMatchesForRequest = async (
  requestId: string,
  requesterId: string,
) => {
  ...
  if (request.requesterId !== requesterId) {
    throw new Error(
      "You don't have permission to view these matches.",
    );
  }
...
const acceptMatch = async (
  ...
  if (match.donorId !== donor.id) {
    throw new Error(
      "You don't have permission to respond to this match.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "This donor match is no longer available.",
    );
  }
  ...
  await prisma.notification.create({
    data: {
      ...
      message:
        `A compatible donor has accepted your blood request at ${match.request.hospitalName}.`,
...
const rejectMatch = async (
  ...
  if (match.donorId !== donor.id) {
    throw new Error(
      "You don't have permission to reject this match.",
    );
  }

  if (match.status === MatchStatus.ACCEPTED) {
    throw new Error(
      "You cannot reject an already accepted match.",
    );
  }
