...
const getDonorApplications = catchAsync(
	async (req: Request, res: Response) => {
		const result =
			await AdminService.getDonorApplications();
...
const getPendingDonorApplications = catchAsync(
	async (req: Request, res: Response) => {
		const result =
			await AdminService.getPendingDonorApplications();

		sendResponse(res, {
			statusCode: 200,
			success: true,
			message:
				"Pending donor applications retrieved successfully.",
			data: result,
		});
	},
);
...
const approveDonorApplication = catchAsync(
	async (req: Request, res: Response) => {
		...
		const result =
			await AdminService.approveDonorApplication({
...
const rejectDonorApplication = catchAsync(
	async (req: Request, res: Response) => {
		...
		const result =
			await AdminService.rejectDonorApplication({
				userId: req.params.userId as string,
				adminId: req.user.userId,
